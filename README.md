...
# Ubuntu / Debian
sudo apt install curl build-essential pkg-config libclang-dev libpam0g-dev zip unzip
curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh
sudo apt install iproute2 podman nmap wget ufw speedtest-cli
```

> **Note:** `libpam0g-dev` is required to compile the PAM authentication module. `libclang-dev` is required by `pam-sys` (bindgen). `pkg-config` is required for build tooling. TLS uses pure-Rust `ring` crypto — no `libssl-dev` needed.
